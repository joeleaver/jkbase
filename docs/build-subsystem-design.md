...
> **Correction 2026-06-19 (the implemented design supersedes the buildpack/language framing in §1, §5a, §5b).** The server build path as actually built is **NOT Cloud Native Buildpacks / Paketo**. It is a **jkbase-owned `jkbuild` lifecycle** (`crates/jkbuild`) with **per-language buildpacks** for **Bun, Node, Rust, Python, and Go**, plus a gated **Dockerfile** escape hatch (`buildah`). Registry order is `bun → node → rust → python → go → dockerfile` (`crates/jkbuild/src/buildpacks/mod.rs`); host-side detection mirrors it (`build_orchestrator.rs::detect_language`), and `tools/dev` bakes a `{lang}.ext4` toolchain + runtime layer for each. **Rust is a first-class _server_ language** — its own buildpack runs `cargo fetch` → `cargo build --release` and emits a server binary; it is **not** WASM-only. (The `cargo build --target wasm32-wasip1` Rust reference in §5b is the separate, deferred *function* path — a different axis.) So wherever §1/§5a/§5b say servers ride "Paketo (Node/Python/Go/Java/.NET/Ruby)," read instead "jkbuild (Bun/Node/Rust/Python/Go + Dockerfile)." (The registry checks Bun before Node only to disambiguate — a Bun repo also carries `package.json` — so its position isn't a ranking.)
...
- **Writable cache — per-project only.** A per-project cache disk (`{data_dir}/buildcache/{project_id}/`, via `create_data_disk`) holds Cargo/npm/pip dirs, the CNB layer cache, and the Go build cache; mounted only into that project's build VM; counts against the project storage quota. For opted-in toolchains (`rust`, `trunk`) only the state captured at the seal persists — see §9 P2-7.
...
- **Transitive-dependency worms (threat-model P2-7).** cosign verifies the buildpack's origin, not the packages it pulls. A fresh Shai-Hulud-class malicious version runs at build before any quarantine exists. **Stated mitigation is the network-off-compile invariant, not quarantine:** even if the worm executes at build, with egress torn down it cannot self-propagate or exfiltrate. Lockfile-digest pinning through the mirror bounds *which* versions run. **Scope as built (2026-09-16): this holds only for code that runs in COMPILE, and only at build time.** The Node buildpack runs install lifecycle scripts and the Python buildpack builds sdists during FETCH, network up — there the egress allowlist + IP pin is the only control; `builder = "dockerfile"` runs every `RUN` step online; and a malicious crate still runs in the tenant's runtime VM, which has egress. For Rust (servers, functions, trunk) it holds: FETCH runs no dependency code; the guest waits for the seal fail-closed (3 consecutive unreachable probes; never "compile anyway" — after 300s it fails the build and powers off); the host verifies the TAP is gone or kills the build; and the per-project cache persists only **as captured at the seal** — the host pauses the VM, seals, copies the cache image as raw bytes, resumes, and discards everything written afterwards (`BuildVmConfig::persist_cache_from_seal`, keyed on the cargo-carrying toolchain images). So a compile-time `build.rs` cannot plant a `$CARGO_HOME` config (`cargo fetch` runs a config-named `rustc-wrapper`) or a git-db hook that runs in the next build's network-up fetch. The cache ext4 has no journal, so the guest must `syncfs` it and print `[seal] CACHE-SYNCED` before the host copies; anything short of a completed capture (no marker, pause/seal/copy failure or timeout, a run that never sealed) persists nothing. A failed fetch still seals, so its cache is captured (no dependency code has run). Pre-capture images were persisted live and are retired (`{key}.sealcap.img` replaces `rust.img`/`trunk.img`). Go is not covered yet (GOCACHE is compile-written on the same drive).
...
- **Multi-target builds — DECIDED (2026-06-05): per-target build VMs.** A project is N servers + M functions (the runtime already runs them all together: `function_runtime` loads M functions, `container_supervisor` runs N servers, and a deploy packs all of them). The build pipeline therefore fans out **one build VM per server and per function**, each with its own minimal content-addressed toolchain image (so CNB's one-app-per-root holds, heterogeneous languages each get the right tools, builds run in parallel and incrementally, and the dedup story — one shared toolchain image per language across all tenants — works). `BuildVm` is the per-target primitive; the fan-out + **atomic assembly** is the build-pipeline core. **Deploy policy: atomic** — any target's build fails → the whole deploy fails (matches `do_deploy`'s atomic symlink swap). Rejected "one fat all-toolchains VM" (poor dedup, CNB conflict, no parallelism). Validate with a 2-function + 1-server fixture repo.
