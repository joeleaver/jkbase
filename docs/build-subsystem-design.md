...
- **Transitive-dependency worms (threat-model P2-7).** cosign verifies the buildpack's origin, not the packages it pulls. A fresh Shai-Hulud-class malicious version runs at build before any quarantine exists. **Stated mitigation is the network-off-compile invariant, not quarantine:** even if the worm executes at build, with egress torn down it cannot self-propagate or exfiltrate. Lockfile-digest pinning through the mirror bounds *which* versions run. **Scope as built (2026-09-16): this holds only for code that runs in COMPILE.** The Node buildpack runs install lifecycle scripts and the Python buildpack builds sdists during FETCH, network up — there the egress allowlist + IP pin is the only control. Rust (servers, functions, trunk) FETCH runs no dependency code, and the per-project cache persists only **as captured at the seal**: the host pauses the VM, copies the cache image as raw bytes, seals, resumes, and discards everything written afterwards (`BuildVmConfig::persist_cache_from_seal`). So a compile-time `build.rs` cannot plant a `$CARGO_HOME` config (`cargo fetch` runs a config-named `rustc-wrapper`) or a git-db hook that runs in the next build's network-up fetch. The guest must `sync` and print `[seal] CACHE-SYNCED` first — a paused copy is only power-cut consistent — and no marker means nothing persists. Go is not covered yet (GOCACHE is compile-written on the same drive).
