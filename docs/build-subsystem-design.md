...
> **Correction 2026-06-19 (the implemented design supersedes the buildpack/language framing in §1, §5a, §5b).** The server build path as actually built is **NOT Cloud Native Buildpacks / Paketo**. It is a **jkbase-owned `jkbuild` lifecycle** (`crates/jkbuild`) with **first-class per-language buildpacks** for **Bun, Node, Rust, Python, and Go** (all equal — jkbase has **no lead language**), plus a gated **Dockerfile** escape hatch (`buildah`). Registry order is `bun → node → rust → python → go → dockerfile` (`crates/jkbuild/src/buildpacks/mod.rs`); host-side detection mirrors it (`build_orchestrator.rs::detect_language`), and `tools/dev` bakes a `{lang}.ext4` toolchain + runtime layer for each. **Rust is a first-class _server_ language** — its own buildpack runs `cargo fetch` → `cargo build --release` and emits a server binary; it is **not** WASM-only. (The `cargo build --target wasm32-wasip1` Rust reference in §5b is the separate, deferred *function* path — a different axis.) So wherever §1/§5a/§5b say servers ride "Paketo (Node/Python/Go/Java/.NET/Ruby)," read instead "jkbuild (Bun/Node/Rust/Python/Go + Dockerfile)." **There is no lead language:** Bun/Node/Rust/Python/Go are equal first-class buildpacks on one lifecycle. Bun was simply the first one implemented; the registry checks it before Node only to disambiguate (a Bun repo also carries `package.json`) — a detection order, not a preference.
