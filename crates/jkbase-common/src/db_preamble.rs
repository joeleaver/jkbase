...
//!
//! This is the **shared wire contract** between the client (`jkbase-cli`, the sidecar)
//! and the edge (`jkbase-proxy`). It lives here so there is exactly ONE definition —
//! a divergence between how the sidecar *encodes* and how the edge *parses* would be a
//! silent auth bypass or a hard-to-diagnose connect failure on a security seam. Only
//! pure byte handling lives here (no I/O, no tokio); the edge's async accumulate-loop
//! (`read_preamble`) stays in `jkbase-proxy`.
//!
...
//!
//! [`parse_preamble`] is a pure function over a byte buffer (no I/O, no allocation of
//! the input) so it is exhaustively unit-testable.
...
/// `ServerConnection`; the client derives the identical value from its side. Both
/// sides MUST pass this exact label and a `None` context to `export_keying_material`,
/// or the const-time compare fails and every connection is rejected.
...
/// Build the preamble wire bytes (the sidecar's send path; also used by tests).
/// Returns `None` if a field is empty or exceeds its cap.
