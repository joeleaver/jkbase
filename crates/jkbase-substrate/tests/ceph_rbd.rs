...
fn token(epoch: u64) -> FenceToken {
    FenceToken { scope: "disk".into(), epoch, holder: "host-a".into(), source_id: "ceph".into() }
}
...
async fn dev_write(path: &str, byte: u8) -> std::io::Result<()> {
    ...
    match tokio::time::timeout(Duration::from_secs(15), fut).await {
        ...
        Err(_) => Err(std::io::Error::new(std::io::ErrorKind::TimedOut, "device write timed out")),
    }
}
...
async fn rbd(args: &[&str]) -> String {
    let out = tokio::process::Command::new("rbd").args(args).output().await.unwrap();
    String::from_utf8_lossy(&out.stdout).into_owned()
}
async fn ceph(args: &[&str]) -> String {
    let out = tokio::process::Command::new("ceph").args(args).output().await.unwrap();
    String::from_utf8_lossy(&out.stdout).into_owned()
}
...
async fn watcher_addr(p: &str, image: &str) -> String {
    rbd(&["status", &format!("{p}/{image}")])
        .await
        .lines()
        .find_map(|l| l.trim().strip_prefix("watcher=").and_then(|r| r.split_whitespace().next()).map(str::to_string))
...
async fn ceph_rbd_lifecycle_and_storage_enforced_fence() {
    ...
    dev_write(&path, 0x5A).await.expect("write to freshly-attached disk");
    assert_eq!(dev_read_first(&path).await, 0x5A, "read-back matches");
    p.detach(&life).await.unwrap();
    p.destroy(&life).await.unwrap();
    assert!(matches!(p.attach_rwo(&life, &token(1)).await, Err(SubstrateError::NotFound(_))));
    ...
    dev_write(&pa, 0xA1).await.expect("holder A writes before being superseded");
    ...
    assert!(blocklist.contains(&watcher_a), "prior writer {watcher_a} must be blocklisted:\n{blocklist}");
