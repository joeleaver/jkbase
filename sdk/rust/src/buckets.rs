...
impl ObjectClient {
    ...
    pub async fn create_bucket(&self, bucket: &str) -> Result<()> {
        self.send(Method::PUT, &format!("/{bucket}"), &[], None, ReqBody::Empty).await?;
        Ok(())
    }

    /// Delete an (empty) bucket. `Err(..).code() == BucketNotEmpty` if it still holds objects.
    pub async fn delete_bucket(&self, bucket: &str) -> Result<()> {
        self.send(Method::DELETE, &format!("/{bucket}"), &[], None, ReqBody::Empty).await?;
        Ok(())
    }
    ...
    pub async fn bucket_exists(&self, bucket: &str) -> Result<bool> {
        match self.send(Method::HEAD, &format!("/{bucket}"), &[], None, ReqBody::Empty).await {
            Ok(_) => Ok(true),
    ...
    pub async fn list_buckets(&self) -> Result<Vec<String>> {
        let resp = self.send(Method::GET, "/", &[], None, ReqBody::Empty).await?;
